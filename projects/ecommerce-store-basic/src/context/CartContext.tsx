...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const add = useCallback(
    (productId: string, qty = 1) => {
      ...
      if (!product || !product.active || !product.available) return;
      const existing = items.find((i) => i.productId === productId);
      const nextQty = Math.min((existing?.quantity ?? 0) + qty, MAX_QTY);
      if (existing) {
        persist(items.map((i) => (i.productId === productId ? { ...i, quantity: nextQty } : i)));
      } else {
        persist([...items, { productId, quantity: nextQty }]);
      }
    },
    [items, persist],
  );
  ...
  const updateQty = useCallback(
    (productId: string, qty: number) => {
      ...
      persist(
        items.map((i) =>
          i.productId === productId ? { ...i, quantity: Math.min(qty, MAX_QTY) } : i,
        ),
      );
    },
    [items, persist],
